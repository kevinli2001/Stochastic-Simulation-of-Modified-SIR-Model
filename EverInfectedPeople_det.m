...
for delta = 0:0.001:1
    ...
    E(1) = I0;
        ...
        for t = 2:T
            ...
            new_infections = (beta*S(t-1)*I(t-1)/P(t-1));
        
            % Calculate the number of death
            new_death = (rho*I(t-1) + rho*Q(t-1));
            
            % Calculate the number of individuals leaving the infected compartment
            new_recoveries = gamma*I(t-1); %-rho*I(t-1)-rho2*I(t-1);
            
            % Calculate the number of individuals leaving the quarantined compartment
            new_quarantines = delta*I(t-1);
            new_dequarantines = gamma*Q(t-1); % number of quarantined individuals who recover
            
            % Update the compartment sizes
            S(t) = S(t-1) - new_infections;
            I(t) = I(t-1) + new_infections - new_recoveries - new_quarantines - rho*I(t-1);
            R(t) = R(t-1) + new_recoveries + new_dequarantines;
            Q(t) = Q(t-1) + new_quarantines - new_dequarantines - rho*Q(t-1);
            D(t) = D(t-1) + new_death;
...
% Plot the result
%subplot(2,1,1),...
    %plot(delta_arr, e_arr, 'r-', 'LineWidth', 2);
    %xlabel('Quarantine rate');
